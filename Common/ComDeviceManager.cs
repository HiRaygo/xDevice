...
namespace xDevice.Common
{
	...
	/// </summary>
	public class ComDeviceManager
	{
		static Dictionary<string, ComDevice> DeviceList = new Dictionary<string, ComDevice>();
		...
		public static bool AddDevice(ComDevice device)
		{
			lock(obj)
			{				
		...
		public static void DeleteALLDevice()
		{
			lock(obj)
			{
				foreach(ComDevice device in DeviceList.Values)
				{
					//停止设备
					device.Stop();
				}
		...
		/// <param name="device"></param>
		/// <returns></returns>
		public static bool GetDeviceByName(string devicename, out ComDevice device)
		{
			lock(obj)
			{
				if(DeviceList.ContainsKey(devicename))
