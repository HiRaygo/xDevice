/*
 * User: XiaoSanYa
 * Date: 2016/2/21
 * Time: 15:13
 */
using System;

namespace xDevice.Common
{
	/// <summary>
	/// Description of CRC.
	/// </summary>
	public class CRC
	{
        // Table of CRC values for high-order byte
        private static readonly byte[] _auchCRCHi = new byte[]
        {
            
        } ;

        // Table of CRC values for low-order byte
        private static readonly byte[] _auchCRCLo = new byte[]
        {
            
        } ;
        
        /// <summary>
		/// CRC16 
		/// </summary>
		/// <param name="data">data</param>
		/// <returns>crc16</returns>
        public static byte[] CRC16( byte[] buffer )
        {
            byte crcHi = 0xff;  // high crc byte initialized
            byte crcLo = 0xff;  // low crc byte initialized

            for ( int i = 0; i < buffer.Length; i++ )
            {
                int crcIndex = crcHi ^ buffer[ i ]; // calculate the crc lookup index

                crcHi = (byte)( crcLo ^ _auchCRCHi[ crcIndex ] );
                crcLo = _auchCRCLo[ crcIndex ] ;
            }

            return new byte[]{crcHi, crcLo};
        }
        
        /// <summary>
		/// CRC16 
		/// </summary>
		/// <param name="data">data</param>
		/// <returns>crc16</returns>
		private static byte[] CRC16X(byte[] data)
		{
		    byte[] returnVal = new byte[2];
		    byte CRC16Lo, CRC16Hi, CL, CH, SaveHi, SaveLo;
		    int i, Flag;
		    CRC16Lo = 0xFF;
		    CRC16Hi = 0xFF;
		    CL = 0x86;
		    CH = 0x68;
		    for (i = 0; i < data.Length; i++)
		    {
		        CRC16Lo = (byte)(CRC16Lo ^ data[i]);
		        for (Flag = 0; Flag <= 7; Flag++)
		        {
		            SaveHi = CRC16Hi;
		            SaveLo = CRC16Lo;
		            CRC16Hi = (byte)(CRC16Hi >> 1);
		            CRC16Lo = (byte)(CRC16Lo >> 1);
		            if ((SaveHi & 0x01) == 0x01)
		            {
		                CRC16Lo = (byte)(CRC16Lo | 0x80);
		            }
		            if ((SaveLo & 0x01) == 0x01)
		            {
		                CRC16Hi = (byte)(CRC16Hi ^ CH);
		                CRC16Lo = (byte)(CRC16Lo ^ CL);
		            }
		        }
		    }
		    returnVal[0] = CRC16Hi;
		    returnVal[1] = CRC16Lo;
		    return returnVal;
		}
		
		/// <summary>
		/// CheckSum 
		/// </summary>
		/// <param name="data">buffer</param>
		/// <returns>CheckSum</returns>
        private static byte CheckSum(byte[] buffer, int iStart, int iEnd)
        {
        	if((iStart < 0)||(iEnd > buffer.Length))
        		throw new Exception("Para Error.");
            byte ret = 0;
            for(int i = iStart; i < iEnd; i++ )
            {
            	ret += buffer[i];
            }
            return ret;
        }
        
		/// <summary>
        /// Add CRC16 to bytes
        /// </summary>
        /// <param name="bufin">data to be add crc</param>
        /// <returns>bytes added crc16</returns>
        public static byte[] AddCRC(byte[] bufin)
        {        	
        	int len = bufin.Length + 2;
        	byte[] rt = new byte[len];
        	for(int i=0; i< len-2; i++)
        	{
        		rt[i] = bufin[i];
        	}
        	
        	byte[] crc16 = CRC16(bufin);
        	rt[len-2] = crc16[0];
        	rt[len-1] = crc16[1];
        	return rt;
        }
        
        /// <summary>
		/// Check Modbus CRC16 
		/// </summary>
		/// <param name="buffer">data</param>
		/// <returns>success?</returns>
        public static bool CheckCRC( byte[] buffer)
        {
            byte crcHi = 0xff;  // high crc byte initialized
            byte crcLo = 0xff;  // low crc byte initialized
			int len = buffer.Length;
            for ( int i = 0; i < len-2; i++ )
            {
                int crcIndex = crcHi ^ buffer[ i ]; // calculate the crc lookup index

                crcHi = (byte)( crcLo ^ _auchCRCHi[ crcIndex ] );
                crcLo = _auchCRCLo[ crcIndex ] ;
            }
            if((crcHi == buffer[len-2]) && (crcLo == buffer[len-1]))
            	return true;
            else
            	return false;
        }
        
        /// <summary>
        /// Calc CRC16 to bytes
        /// </summary>
        /// <param name="buffer">data to be add crc</param>
        public static void CalcCRC(byte[] buffer)
        {        	
        	byte crcHi = 0xff;  // high crc byte initialized
            byte crcLo = 0xff;  // low crc byte initialized
			int len = buffer.Length;
            for ( int i = 0; i < len-2; i++ )
            {
                int crcIndex = crcHi ^ buffer[ i ]; // calculate the crc lookup index

                crcHi = (byte)( crcLo ^ _auchCRCHi[ crcIndex ] );
                crcLo = _auchCRCLo[ crcIndex ] ;
            }
            buffer[len-2] = crcHi;
            buffer[len-1] = crcLo;
        }
	}
}
